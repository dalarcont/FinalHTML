...
class StudentPortal implements I_StudentsApps{
    ...
    private function EncSession($a,$b){
        $Head = "U"; $Tail = "F"; $Spacer = "UN1F4L54";
        $EncKey = $Head.base64_encode($a).$Spacer.base64_encode($b).$Tail;
        return $EncKey;
    }
    private function DecSession($e){
        return array(
    ...
    public function __construct(){
        ...
        $this->RootSistemas = new variablesAcademicasGenerales();
        $this->DataBaseConnection = new Root_BasesDatos();
        ...
        if(!empty($ArrayAcceso)){
            ...
            //Verificar que esté habilitado para ingreso:
                switch($ArrayAcceso["ESTADOGENERAL"]){
                    case 6:
                        //Es primíparo
                        if($this->appset_AccessCriteria()!=false){
                            ...
                            echo "<b style='color:red;'>Oh! <big>PRIMÍPARO! Bienvenido!</big><br><br>Por favor espere...</b>";
    ...
    public function appset_LoggedUserData($e){
        ...
        $a = $this->DecSession($e)[0]; $b = $this->DecSession($e)[1];
    ...
    public function appset_horarioClases(){
        ...
        for($asignatura=0; $asignatura<=(sizeof($data)-1); $asignatura++){
            ...
            array_push($asg,$data[$asignatura]["codigoLiteral"]);
            ...
            for($dia=5; $dia<=11; $dia++){
                if(!empty($data[$asignatura][$dia])){
                    ...
                    if($dia==5){
                        ...
                        array_push($dataRenderHorario["Lunes"],$horarioDia);
                    }
                    if($dia==6){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Martes"],$horarioDia);
                    }
                    if($dia==7){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Miercoles"],$horarioDia);
                    }
                    if($dia==8){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Jueves"],$horarioDia);
                    }
                    if($dia==9){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Viernes"],$horarioDia);
                    }
                    if($dia==10){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Sabado"],$horarioDia);
                    }
                    if($dia==11){
                        //Guardar configuracón
                        array_push($dataRenderHorario["Domingo"],$horarioDia);
                    }
                }
            }
        }  //Obtener datos
